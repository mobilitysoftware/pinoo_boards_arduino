...
bool Arduino_MQTT_Client::set_buffer_size(const uint16_t& buffer_size) {
    // TBPubSubClient uses separate receive/send buffers; use buffer_size for both
    return m_mqtt_client.setBufferSize(buffer_size, buffer_size);
}

uint16_t Arduino_MQTT_Client::get_buffer_size() {
    // TBPubSubClient exposes getSendBufferSize() instead of getBufferSize()
